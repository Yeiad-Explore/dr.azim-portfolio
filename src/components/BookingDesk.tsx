...
export function BookingDesk() {
  ...
  function validateLocally(): string | null {
    ...
    if (step === "when") {
      ...
      if (Date.parse(toDhakaIso(when)) < Date.now())
        return "That time is in the past. Choose a later slot.";
    }
    return null;
  }
  ...
  function advance() {
    ...
    setError(null);
    setStep(
      step === "service" ? "patient" : step === "patient" ? "when" : "review",
    );
  }
    ...
    try {
      const res = await fetch("/api/booking", {
        ...
        body: JSON.stringify({
          ...
          service,
          preferred: {
            start: startIso ?? toDhakaIso(when),
            durationMinutes: 30,
          },
  ...
  const stepIndex = { service: 1, patient: 2, when: 3, review: 4, result: 4 }[
    step
  ];
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
          {error && (
            <p
              role="alert"
              className="mb-4 border-l-2 border-accent-deep pl-3 text-[0.875rem] text-accent-deep"
            >
              {error}
            </p>
          )}
          ...
          {step === "service" && (
            <fieldset>
              <legend className="mb-3 text-[0.9375rem] text-ink">
          ...
          {step === "when" && (
            <div className="space-y-3">
              ...
              <div>
                <label
                  htmlFor="bk-notes"
                  className="mb-1 block font-mono text-[0.625rem] tracking-[0.14em] text-ink-faint"
                >
          ...
          {step === "result" && result && (
            <div className="space-y-4">
              {result.status === "confirmed" && (
                <>
                  <p className="font-mono text-[0.6875rem] tracking-[0.16em] text-accent-deep">
                    CONFIRMED
                  </p>
                  <dl className="space-y-2.5 font-mono text-[0.8125rem]">
                    <Row
                      label="REF"
                      value={result.details.appointmentId ?? "—"}
                    />
                    ...
                    <Row
                      ...
                      value={
                        result.details.start
                          ? formatSlot(result.details.start)
                          : "—"
                      }
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    {result.alternate_slots.length === 0 && (
                      <p className="text-[0.875rem] text-ink-secondary">
                        No open slots were returned. Please call{" "}
                        {PRACTICE_PHONE}.
                      </p>
                    )}
                  </div>
              ...
              {result.status === "error" && (
                <>
                  ...
                  <p className="font-mono text-[0.8125rem]">
                    <a
                      href={`tel:${PRACTICE_PHONE.replace(/\s/g, "")}`}
                      className="text-link"
                    >
                      {PRACTICE_PHONE}
                    </a>
...
function Field({
  ...
  return (
    <div>
      ...
      />
      {hint && (
        <p className="mt-1 font-mono text-[0.625rem] text-ink-faint">{hint}</p>
      )}
    </div>
  );
}
